...
举个例子：`cat` => `猫`。单词软件常常会给你提供某个具体的关于猫的图片，但真正值得记住的不是那一张图片，而是你脑海里能够灵活调用的「猫」这个概念：

![抽象化记忆单词](../../assets/vocabulary-cat-concepts.svg)
...
![单词本示意图](../../assets/单词本示意图.jpg)

这个习惯我一直保留至今(从 2010 年 12 月开始)，只不过我不再用纸质的本子，而是将其同步到了词典软件上，每天午休期间和晚上健身后翻翻。
...
因为上面卡牌包制作时间久远，在一些移动平台曾出现样式兼容问题，更新建议如下：
...
本章前面讲的原则在 2026 年依然适用：**单词要放在语境里学**，而不是只背「单词=中文释义」的键值对。不同的是，现在你多了一件趁手的工具——对话式 AI（如 ChatGPT 等），可以帮你更快地生成语境和练习，但记忆这件事仍然只能由你自己完成。
...
> 如果你想系统地把 AI 融入听说读写的学习中，可以配合阅读本指南中的「用 AI 学英语」章节。
